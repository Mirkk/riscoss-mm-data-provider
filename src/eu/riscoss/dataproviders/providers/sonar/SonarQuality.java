 ...
 * 
 */
package eu.riscoss.dataproviders.providers.sonar;

import java.util.ArrayList;
import java.util.Arrays;
import eu.riscoss.dataproviders.providers.sonar.SonarDataProvider.SonarStatistics;
...
public class SonarQuality {
	 ...
	 * @param stats
	 * @param metricNames
	 * @param classvalueslist
	 */
	public static void createIndicators_Quality_PerClass(SonarStatistics stats, String[] metricNames, ArrayList<ClassInfo> classvalueslist) {
		...
		if (classvalueslist!=null && classvalueslist.size()!=0) {
			if (Arrays.asList(metricNames).contains("complexity") && Arrays.asList(metricNames).contains("ncloc")) {
				...
				// double
				if (complexityPercentage[0] <= 0.25 && complexityPercentage[1] < 0.01 && complexityPercentage[2] < 0.01)
					cRank = 2;
 ...
 */
	public static void createIndicators_Quality_Java(SonarStatistics stats) {
		...
		if (stats.singleValues.containsKey("coverage")) {
			...
			double dd = 0; //coverage
			double coverage_rank = 0;
			
			if ((dd = (stats.singleValues.get("line_coverage"))) >= 0) {
	 ...
	 * Note: Changed to calc values on single files instead of ncloc.
