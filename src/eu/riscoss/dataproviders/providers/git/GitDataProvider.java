...
public class GitDataProvider implements AbstractDataProvider
{
     ...
     */
    protected GitDataProvider.GitLogStatistics getStatistics()
    {
        ...
        try {
            ...
            for (String commitId : commitIds) {
                commitDetails.add(getCommitDetails(commitId));
            }
     ...
     * @return
     */
    private double[] getDistribution(int[] values, int total)
    {
        double[] distribution = new double[values.length];
        for (int i = 0; i < values.length; i++) {
            distribution[i] = (double) values[i] / total;
        }
        return distribution;
    }
