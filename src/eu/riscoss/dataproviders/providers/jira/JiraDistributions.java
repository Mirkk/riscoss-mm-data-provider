...
import eu.riscoss.dataproviders.common.RiskDataUtils;
import eu.riscoss.dataproviders.providers.jira.JiraDataProvider.JiraLogStatistics;
...
public class JiraDistributions {
	...
	/**
	 * Calculates various distributions and stores them in an internal table.
	 * Needed distributions to be added here.
	 */
...
//		final String[] Levels_commit_Hours = {, "medium", "high"}; //Need to interpret "low/medium/high" for hours!
		final Map<String, String[]> Levels_commit_Hours = new LinkedHashMap <String, String[]>() {{//Need to interpret "low/medium/high" for hours!
			...
			put("high", new String[] {"21", "22", "23", "24", "0", "1", "2", "3", "4", "5", "6"}); //format should be 1..24
		}};
		
		final Map<String, String[]> Levels_commit_Weekday = new LinkedHashMap <String, String[]>() {{//Need to interpret "low/medium/high" for hours!
...
//		}};
		...
		distributions.put("VAverage_bug_fix_time__days_", RiskDataUtils.getNumberDistribution(stats.list_bugFixTime, Levels_bugFixTime_Days));

		distributions.put("VBug_fix_time_for_critical___blocker_level_bugs", RiskDataUtils.getNumberDistribution(stats.list_bugFixTimeCriticalBlocker, Levels_bugFixTimeCriticalBlocker_Days));
		
		distributions.put("VCommit_frequency___week", RiskDataUtils.getNumberDistributionInt(new ArrayList<Integer>(stats.list_commit_frequency_week.values()), Levels_commitFrequency_Week));
		...
		Map<String, Double> weekdays = RiskDataUtils.getStringDistribution(stats.list_GeneralFixDayOfWeek, Levels_commit_Weekday);
		...
		distributions.put("VWeekday__When_the_commit_was_made", new Distribution(weekdays.values().toArray(new Double[0])));
	}
